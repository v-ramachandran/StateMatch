...
}function drawLine3(state) {
d3.csv("preprocessed-data/CrimeRate1990-2009.csv", function handleCSV(csv) {
      points = csv.filter(function(el) { return el.State == state });
      temp = [];
      for(i=1990;i<2010;i++){
        temp.push(parseFloat(points[0][i])/1000000);
      }
      points = temp;
      //          .map(function(el) { return parseFloat(el.Male_le); }); 
      console.log(points);
